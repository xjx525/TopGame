...
*$.sobox.pop({ 
	...
	* 位置信息 *
	posType:'center',//'center,set' 位置类型，默认居中显示，可自定义距离浏览器顶部坐标@
	...
	* 自定义参数 *
	...
	btn : [],//{cls:,name:'确定',link:,returnFalse:true,closePop: true,callback:}@

	* 返回事件 *
	beforePop:function(){},//弹窗打开之前callback事件@
	onPop: function(){},//页面打开callback事件@
	closePop: function(){}//s-sopop-close点击返回事件@
*});
...
(function ($,document) {
	$.sobox = {
		...
		showMask : function (state) {
			var that = this;
			var $mask = $('.so-openmask'),dh = $(document).height();
			if (!$mask.length){
		...
		pop : function (o) {
			...
			if (o.defaultShow) {showPop();}//默认直接显示，,否则通过手动事件执行showPop，初始化显示pop

			function showPop(opt) {//showPop
				o = $.extend(o,opt||{});
				if (o.showTitle) {//增加title
					$wrap.append($title);
				}else {
					$close.addClass('s-sopop-single-close');//添加 无标题时 close按钮样式
				}
				...
				if (o.type == 'iframe') {//iframe模式
					...
					var iframeHtml = $('<iframe src="'+o.iframe+'" width="100%" height="'+o.height+'" frameborder="0" scrolling="auto"></iframe>');
				...
				o.beforePop($wrap,$title,$close,$cont);//赋予before事件所有可操作对象
				 $mask = that.show({
					...
					pos : o.pos,
					offset: o.offset
				},o.showMask,o.onPop(removePop));//显示弹出层，并设置$mask为背景层
				$close.bind('click',function () {o.closePop();removePop();});
		...
		confirm : function (title,content,success,cancel) {//确认框：快捷4个参数：标题、正文、点击确定返回事件、点击取消返回事件
			...
			that.pop({
				cls:'so-popConfirm',
				title :title,width:320,content :content,
...
*/
	$.fn.soIframePop = function (opt) {//采用链接的方式直接定义iframe目标和参数
		var o = $.extend({
			type: 'iframe',
...
*/
	...
	$.fn.soSidePop = function (opt) {//侧边pop
		var _self = $(this);
		...
		_self.bind(ev,function (e) {
			var o = $.extend({
				...
				pos : [e.pageX,e.pageY],//取当前鼠标坐标为定位坐标
